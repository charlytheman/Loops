...
let food = document.getElementsByClassName("recipes")[0];
...
for (let n = 0; n <= 3; n++) {
    let recipe = recipes[n];
    recCont = document.createElement("div");
    recCont.classList.add("recipe");
   
    // creating the title name 
    // arranging the title names in columns as seen on output 
    let title = document.createElement("h2");
    title.innerText = recipe.title;
	
    // makes time bold
    // gives a value to time
    let time = document.createElement("p");
	time.innerHTML = `<b>Time:</b> ${recipe.time}`;
 
	
    // makes calories bold
    // gives a value to calories 
    let calories = document.createElement("p");
	calories.innerHTML = `<b>Calories: </b> ${recipe.calories}`;
   
    // makes ingredients bold
    ...
    recCont.appendChild(time);
    recCont.appendChild(calories);
    recCont.appendChild(ingredients);
    food.appendChild(recCont);
}
